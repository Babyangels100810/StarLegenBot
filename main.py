...
def checkout_start(c: types.CallbackQuery):
    ...
    CHECKOUT_STATE[uid] = {
        ...
        "order": {
            ...
            "address": "",
            "comment": "",
            "items": [{"code": x, "qty": q} for x, q in CART[uid].items()],
...
def back_main_msg(m: types.Message):
    try:
        go_home(m)   # ← սա քո գոյություն ունեցող ֆունկցիան է (Գլխավոր մենյու)
    except Exception:
        # եթե go_home չկարողանա, fallback
        main = types.ReplyKeyboardMarkup(resize_keyboard=True)
        main.add("🛍 Խանութ", "🛒 Զամբյուղ")
        main.add("📦 Իմ պատվերները", "🔍 Ապրանքների որոնում")
        main.add("🧍 Իմ էջը", "💱 Փոխանակումներ")
        main.add("💬 Կապ մեզ հետ", "💡 Խոհուն մտքեր")
        main.add("📢 Բիզնես գործընկերներ", "📈 Օրվա կուրսեր")
        main.add("👥 Հրավիրել ընկերների")
...
def back_main_cb(c: types.CallbackQuery):
    try:
        go_home(c.message)  # նույնը callback-ով
    except Exception:
        back_main_msg(c.message)
    bot.answer_callback_query(c.id)
...
def _slider_kb(code: str, idx: int, total: int):
    ...
    # զամբյուղի կոճակներ
    add_btn = types.InlineKeyboardButton("➕ Ավելացնել զամբյուղ", callback_data=f"cart:add:{code}")
    show_btn = types.InlineKeyboardButton("🧺 Դիտել զամբյուղ", callback_data="cart:show")

    # վերադարձ
    back_list = types.InlineKeyboardButton("⬅️ Վերադառնալ ցուցակ", callback_data="back:home_list")
    gohome = types.InlineKeyboardButton("🏠 Գլխավոր մենյու", callback_data="go_home")

    kb = types.InlineKeyboardMarkup()
    kb.row(left, right)
    kb.row(add_btn, show_btn)
    kb.row(back_list, gohome)
    return kb
...
# ───────── CART ՀԵՆԴԼԵՐ ─────────
@bot.callback_query_handler(func=lambda c: c.data == "checkout:start")
def on_checkout_start(c: types.CallbackQuery):
    uid = c.from_user.id
    ...
    if not ok:
        ...
        try:
            bot.send_message(
                c.message.chat.id,
                f"⚠️ {PRODUCTS[code]['title']} — հասանելի՝ {st} հատ"
            )
        except Exception:
            pass
        return

    # 3) Ստեղծում ենք պատվերի state
    order_id = _order_id()
    CHECKOUT_STATE[uid] = {
        "step": "name",
        "order": {
    ...
    # 4) Հանում ենք cart-ի inline клавиатураն, որ չկրկնքլինի սեղմումը
    try:
        bot.edit_message_reply_markup(c.message.chat.id, c.message.message_id, reply_markup=None)
    except Exception:
        pass

    # 5) Սկսում ենք checkout wizard-ը՝ Անուն Ազգանուն
    kb = types.ReplyKeyboardMarkup(resize_keyboard=True, one_time_keyboard=True)
    kb.add(BTN_BACK_MAIN)  # ⬅ Վերադառնալ գլխավոր մենյու
    bot.answer_callback_query(c.id)
    bot.send_message(
        c.message.chat.id,
        f"🧾 Պատվեր {order_id}\nԳրեք ձեր **Անուն Ազգանուն**:",
...
if __name__ == "__main__":
    ...
    # ❗ կարևոր՝ երբեմն 409 conflict է լինում, եթե webhook է միացած կամ
    # մի քանի անգամ ես գործարկում bot.infinity_polling-ը:
    try:
        bot.delete_webhook(drop_pending_updates=True)
    except Exception:
        pass

    # allowed_updates -> telebot.util-ը
